...
function initializeContentScript() {
  ...
  try {
    ...
    window.__ghostKeyContentLoaded = true;
...
async function startPageVoiceRecording() {
  try {
    ...
    if (__gk_pageMediaRecorder && __gk_pageMediaRecorder.state === 'recording') {
      console.log('Recording already in progress');
      return;
    }

    // Request microphone permission directly (more reliable)
    __gk_pageChunks = [];
    
    try {
      __gk_pageMediaStream = await navigator.mediaDevices.getUserMedia({ 
        audio: {
      ...
      if (getUserMediaError.name === 'NotAllowedError' || getUserMediaError.name === 'PermissionDeniedError') {
        throw new Error('Microphone permission denied. Please allow microphone access for this site in your browser settings.');
    ...
    __gk_pageMediaRecorder = new MediaRecorder(__gk_pageMediaStream, { 
      mimeType: 'audio/webm;codecs=opus' 
    });
    
    __gk_pageMediaRecorder.ondataavailable = e => { 
      if (e.data && e.data.size > 0) {
        __gk_pageChunks.push(e.data);
      }
    };
    
    __gk_pageMediaRecorder.onerror = (e) => {
      ...
      try {
        ...
        switch (message.type) {
          ...
          case 'POPUP_STOP_VOICE_RECORD':
            (async () => {
              try {
                const audioBlob = await stopPageVoiceRecording();
                try {
                  const buf = await audioBlob.arrayBuffer();
                  const bytes = Array.from(new Uint8Array(buf));
                  // Send sample result back to extension (popup will listen)
                  chrome.runtime.sendMessage({ 
                    type: 'VOICE_SAMPLE_RESULT', 
                    payload: { bytes, mime: audioBlob.type } 
                  }).catch(err => {
                    console.warn('Failed to send voice sample to popup:', err);
                  });
                  sendResponse({ success: true });
                } catch (e) {
