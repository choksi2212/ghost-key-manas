...
async function toggleVoiceRecording() {
  ...
  if (btn.textContent.includes('Start')) {
    ...
    try {
      // Try content script first, fallback to direct recording
      await startPageRecordingViaContent();
    } catch (err) {
      console.warn('Content script recording failed, trying direct recording:', err);
      try {
        await startDirectRecording();
      } catch (directErr) {
        console.error('Direct recording also failed:', directErr);
        btn.textContent = '🎤 Start Recording';
        btn.disabled = false;
        stopVoiceTimer();
        alert('Unable to start recording. Please:\n1. Allow microphone permission when prompted\n2. Make sure you\'re on a website (not chrome:// pages)\n3. Try again');
        return;
      }
    }
    ...
    try {
      await stopPageRecordingViaContent();
    } catch (err) {
      console.warn('Content script stop failed, trying direct stop:', err);
...
async function startPageRecordingViaContent() {
  try {
    ...
    // Skip content script for chrome:// pages or extension pages
    ...
    try {
      const results = await chrome.scripting.executeScript({
        target: { tabId: tab.id },
        func: () => {
          return typeof window.__ghostKeyContentLoaded !== 'undefined';
        }
      });
      contentScriptLoaded = results && results[0] && results[0].result === true;
    } catch (e) {
      console.log('Could not check content script status:', e);
    ...
    const response = await Promise.race([
      new Promise((resolve, reject) => {
        chrome.tabs.sendMessage(tab.id, { type: 'POPUP_START_VOICE_RECORD' }, (res) => {
          ...
          reject(new Error(res && res.error || 'Failed to start recording'));
        });
      }),
      new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Timeout: Content script did not respond')), 5000);
      })
    ]);
...
async function startDirectRecording() {
  try {
    ...
    if (!navigator.mediaDevices || !navigator.mediaDevices.getUserMedia) {
      throw new Error('Microphone access not available in this context. Please use the content script recording.');
    }
    ...
    try {
      ...
      if (getUserMediaError.name === 'NotAllowedError' || getUserMediaError.name === 'PermissionDeniedError') {
        ...
        throw new Error('Failed to access microphone: ' + getUserMediaError.message);
      }
    }
    ...
    // Try to create MediaRecorder with preferred codec, fallback to default
    ...
    console.log('Direct recording started in popup with mimeType:', voiceMediaRecorder.mimeType);
  } catch (error) {
...
async function stopPageRecordingViaContent() {
  try {
    // If direct recording is active, stop it
    if (voiceMediaRecorder && voiceMediaRecorder.state !== 'inactive') {
      voiceMediaRecorder.stop();
      return;
    }
    ...
    const response = await Promise.race([
      new Promise((resolve, reject) => {
        chrome.tabs.sendMessage(tab.id, { type: 'POPUP_STOP_VOICE_RECORD' }, (res) => {
          ...
          reject(new Error(res && res.error || 'Failed to stop recording'));
        });
      }),
      new Promise((_, reject) => {
        setTimeout(() => reject(new Error('Timeout: Content script did not respond')), 5000);
    ...
    // If direct recording is active, stop it anyway
    if (voiceMediaRecorder && voiceMediaRecorder.state !== 'inactive') {
      voiceMediaRecorder.stop();
    }
  }
...
async function completeRegistration() {
  const wrappedFunction = window.GhostKeyRecovery ? 
    window.GhostKeyRecovery.withErrorHandling(async () => {
      ...
      // Ensure profile is fully JSON-serializable before saving
      const serializedProfile = JSON.parse(JSON.stringify(newProfile));
      popupState.profiles[profileId] = serializedProfile;
